...
| node\_locations | (Optional) The list of zones in which the cluster's nodes are located.<br>  Nodes must be in the region of their regional cluster or in the same region as their cluster's zone for zonal clusters.<br>  If this is specified for a zonal cluster, omit the cluster's zone. | `list(string)` | n/a | yes |
